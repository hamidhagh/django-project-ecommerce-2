...
#         products = OrderItem.objects.filter(pk=kwargs['pk'])
#         form = CouponForm()
...
#             cart = Cart(request)
#             for c in cart:
#                 OrderItem.objects.create(order_id=order.id, user_id=request.user.id, product_line=c['product_line'],
#                                          quantity=c['quantity'])
...
#             try:
#                 coupon = DiscountCode.objects.get(code__iexact=code, start__lte=time, end__gte=time, active=True)
