...
#         self.product_line = ProductLine.objects.filter(available=True, product_id=kwargs['pk'], )
#         return super().setup(request,*args,**kwargs)

#     def get(self, request, *args, **kwargs):
#         product_lines = get_object_or_404(ProductLine, id=self.product_line[0].id)
#         size = ProductLine.objects.raw('SELECT * FROM home_variants WHERE product_id=%s GROUP BY size_id',
#                                     [self.kwargs['pk']])
#         # size = ProductLine.objects.filter(available=True, product_id=self.kwargs['pk']).distinct('size_id')
#         colors = ProductLine.objects.filter(available=True, product_id=self.kwargs['pk'], size_id=self.product_line[0].size_id)
#         context = (
#             {'products': self.products, 'related_products': self.related_products,
#              'product_lines': self.product_line, 'product_lines': product_lines, 'colors': colors, 'size': size,
#              'comments': self.comments, 'change': self.change, 'wishlist': self.wishlist})
#         return render(request, 'home/details.html', context)

#     def post(self, request, *args, **kwargs):
#         pl_id = self.request.POST.get('select')
#         product_lines = get_object_or_404(ProductLine, available=True, id=pl_id)
#         size = ProductLine.objects.raw(
#             'SELECT * FROM home_variants WHERE product_id=%s GROUP BY size_id', [self.kwargs['pk']])
#         colors = ProductLine.objects.filter(available=True, product_id=self.kwargs['pk'], size_id=product_lines.size_id)
#         # size = ProductLine.objects.filter(available=True, product_id=self.kwargs['pk']).distinct('size_id')
#         context = ({'products': self.products, 'related_products': self.related_products, 'product_line': self.product_line,
#                     'product_lines': product_lines, 'colors': colors, 'size': size, 'comments': self.comments,
